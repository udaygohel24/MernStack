...
const Products = () => {
  ...
  const ProductList = async () => {
    let result = await fetch("https://localhost:4000/products", {
      headers: {
  ...
  const HeandleDelete = async (id) => {
    let result = await fetch(`https://localhost:4000/delete/${id}`, {
      method: "DELETE",
      headers: {
        authorization: `bearer ${JSON.parse(localStorage.getItem("token"))}`,
      },
  ...
  const HeandleSearch = async (e) => {
    ...
    if (key) {
      let result = await fetch(`https://localhost:4000/search/${key}`, {
        headers: {
          authorization: `bearer ${JSON.parse(localStorage.getItem("token"))}`,
        },
      });
